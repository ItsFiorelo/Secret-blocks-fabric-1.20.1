...
public class SecretBlockEntity extends BlockEntity implements NamedScreenHandlerFactory, ImplementedInventory, BlockEntityTicker<SecretBlockEntity> {
	...
	private RegistryEntryLookup<Block> getBlockLookup() {
		if (this.world != null) {
			return this.world.getRegistryManager().getWrapperOrThrow(RegistryKeys.BLOCK);
		}
		return BuiltinRegistries.createWrapperLookup().getWrapperOrThrow(RegistryKeys.BLOCK);
	}

	private static RegistryEntryLookup<Block> getStaticBlockLookup() {
		return BuiltinRegistries.createWrapperLookup().getWrapperOrThrow(RegistryKeys.BLOCK);
	}

	private BlockState readBlockState(NbtCompound tag, String key) {
		return NbtHelper.toBlockState(getBlockLookup(), tag.getCompound(key));
	}

	private static BlockState readStaticBlockState(NbtCompound tag, String key) {
		return NbtHelper.toBlockState(getStaticBlockLookup(), tag.getCompound(key));
	}
	...
	public void deserialize(NbtCompound tag) {
		...
		if (tag.contains("upState")) {
			this.upState = readBlockState(tag, "upState");
		}
		if (tag.contains("downState")) {
			this.downState = readBlockState(tag, "downState");
		}
		if (tag.contains("northState")) {
			this.northState = readBlockState(tag, "northState");
		}
		if (tag.contains("eastState")) {
			this.eastState = readBlockState(tag, "eastState");
		}
		if (tag.contains("southState")) {
			this.southState = readBlockState(tag, "southState");
		}
		if (tag.contains("westState")) {
			this.westState = readBlockState(tag, "westState");
		}
	...
	public static BlockState getState(Direction dir, NbtCompound tag) {
		...
		switch (dir) {
			case DOWN:
				if (tag.contains("downState")) {
					tempState = readStaticBlockState(tag, "downState");
				}
				break;
			case NORTH:
				if (tag.contains("northState")) {
					tempState = readStaticBlockState(tag, "northState");
				}
				break;
			case EAST:
				if (tag.contains("eastState")) {
					tempState = readStaticBlockState(tag, "eastState");
				}
				break;
			case SOUTH:
				if (tag.contains("southState")) {
					tempState = readStaticBlockState(tag, "southState");
				}
				break;
			case WEST:
				if (tag.contains("westState")) {
					tempState = readStaticBlockState(tag, "westState");
				}
				break;
			case UP:
			default:
				if (tag.contains("upState")) {
					tempState = readStaticBlockState(tag, "upState");
				}
				break;
		}
		return tempState;
	...
	public void renderBlock(BlockRenderView blockView, BlockState state, BlockPos pos, Supplier<net.minecraft.util.math.random.Random> randomSupplier, RenderContext context) {
		...
		for (Direction direction : Direction.values()) {
			...
			if (tag.contains(direction.name().toLowerCase() + "State")) {
				tempState = readBlockState(tag, direction.name().toLowerCase() + "State");
			}
			...
			// Keep the dedicated glass face for One Way Glass.
			// The old check turns any non-solid face back into stone, which also
			// kills vanilla glass because glass is not side-solid.
			if (tempState.getBlock() != Blocks.GLASS && !tempState.isSideSolidFullSquare(blockView, pos, tempDirection)) {
